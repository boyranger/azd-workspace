...
CONTAINER_MEMORY_GB=${CONTAINER_MEMORY_GB:-0.5}
...
az acr build \
  --registry "$ACR_NAME" \
  --image "zeroclaw:$IMAGE_TAG" \
  --file src/zeroclaw/DOCKERFILE \
  src/zeroclaw
...
az container create \
  ...
  --os-type Linux \
  --cpu "$CONTAINER_CPU" --memory "$CONTAINER_MEMORY_GB" \
  --registry-login-server "$ACR_LOGIN" \
  --registry-username "$ACR_NAME" \
  --registry-password "$ACR_PASSWORD" \
  --dns-name-label "$DNS_LABEL" \
  --ports 8080 \
  --environment-variables DATABASE_URL="sqlite:///data/zeroclaw.db" ZEROCLAW_MASTER_KEY="${ZEROCLAW_MASTER_KEY:-local_master}" ZEROCLAW_SALT_KEY="${ZEROCLAW_SALT_KEY:-local_salt}" \
