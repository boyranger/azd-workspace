...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -x|--external-db) EXTERNAL_DB_CONN="$2"; shift 2 ;;
...
# If not using external DB, ask for DB admin password
if [ -z "${EXTERNAL_DB_CONN-}" ]; then
  if [ -z "${DB_ADMIN_PASS-}" ]; then
    echo "No external DB connection string provided. A PostgreSQL server will be provisioned."
